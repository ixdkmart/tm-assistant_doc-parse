...
export async function enforceRateAndBudget(opts: { approxTokens: number }) {
  const now = new Date();
  ...
  if (s.estUsdMonth + usd > config.monthlyBudgetUSD) {
    throw new Error(
      `Monthly budget guard: est $${(s.estUsdMonth + usd).toFixed(2)} would exceed $${config.monthlyBudgetUSD}.`
    );
  }
