...
export async function runPipeline(basePath: string = "processing"): Promise<PipelineResult> {
    ...
    // Step 3: FORMAT 03 → 04 (from NDJSON to formatted AKO files)
    const step3Input = path.resolve(basePath, "03-atomisation-out");
    const step3Docs = path.resolve(basePath, "02-clean-out");
    const step3Output = path.resolve(basePath, "04-merge-out");
    stepsBar.update({ step: `3: FORMAT 03-atomisation-out → 04-merge-out` });
    console.log(`\n▶️  Step 3: FORMAT 03-atomisation-out → 04-merge-out`);
