...
export async function convertFolder(inDir: string, outDir: string) {
    ...
    const bar = new cliProgress.SingleBar(
      { format: `Processing {bar} {value}/{total} | ETA: {eta_formatted} | File: {file}`, hideCursor: true },
    ...
    const totalChunks = results.reduce((sum, r: any) => sum + (r.chunks ?? 0), 0);
