## Page 1

Loadfill
Adhoc Replenishment

[Screen display shows:]
Progress: 0/1

Current time
1 min

Scan to complete

C1
H-Cage: UN11111111
In progress

v3.5

[Kmart logo in top right corner with tagline "Low prices, for life"]

[Image shows a Zebra handheld device displaying the Loadfill interface]

## Page 2

Loadfill (Adhoc Replenishment) - Accessing Loadfill

1. Select Menu
2. Select the Loadfill under Adhoc replenishment

[Screen elements shown:
- Store tasks header with Kmart 1136 • Friday, 19 Dec
- Menu and Filter buttons
- Menu options including:
  - Decant
    - Cage scan
  - Ad-hoc replenishment
    - Loadfill]

v3.5

## Page 3

Loadfill (Adhoc Replenishment) – Completing a cage, rail, pallet

1. Scan the barcode on the equipment to be filled
- Screen shows "Scan barcode to start" with "Let's go!" message
- Progress: 0/0

2. Select Start loadfill cage
- Shows "Scanned to C1"
- H-Code: UN11111111
- "Edit area" option available
- "Start loadfill cage" button at bottom

3. Scanned equipment type will be shown, and status will be in progress
- Shows C1 "In progress"
- H-Code: UN11111111
- Current time: 1 min
- Progress: 0/1

4. Fill scanned equipment type

5. Completed equipment type and area will show as done, pop message will also display done. Continue to repeat the steps as you complete each type of equipment to be filled.
- Shows C1 "Completed in 14 mins"
- H-Code: UN11111111
- Current time: 0 min
- Progress: 1/1
- Pop-up message: "Nailed it. C1 cage is done."

v3.5

## Page 4

Loadfill (Adhoc Replenishment) – Completing multiple cages, rails, pallets

1. Scan the barcode on the equipment to be filled
- Progress: 1/1
- Current time: 0 min
- C1 Completed in 14 mins (Done)
- H-Code: UN11111111

2. Select Start loadfill cage
- Scanned to C2
- H-Code: UN20052222
- [Start loadfill cage] button

3. Scanned equipment type will be shown, and status will be in progress
- Progress: 1/2
- Current time: 1 min
- C2 (In progress)
- H-Code: UN22222222
- C1 Completed in 14 mins (Done)
- H-Code: UN11111111

4. Fill scanned equipment type

5. Completed equipment type and area will show as done, pop message will also display done. Completed equipment types will all be listed in the loadfill (Adhoc Replenishment) screen
- Progress: 2/2
- Current time: 0 min
- C2 Completed in 15 mins (Done)
- H-Code: UN22222222
- C1 Completed in 14 mins (Done)
- H-Code: UN11111111
- Message: "Great! C2 cage is done."

v3.5

## Page 5

Loadfill (Adhoc Replenishment) - Area not known

1. Scan the barcode on the equipment to be filled
Progress: 0/0
"Let's go!"

2. No area selected will be shown. Select Start loadfill cage.
H-Code: UN20052222

3. Select the world
H-Code: UN44444444
Options:
- Home
- Kids
- Clothing
- Event

4. Select the area
H-Code: UN44444444
Options:
C1: Womenswear, WUSSH
C2: Menswear, Basics
C3: Footwear
C4: Beauty & Accessories

v3.5

## Page 6

Loadfill (Adhoc Replenishment) - Area not known continued

5. Equipment type will have the area selected. Select "Start loadfill cage"

6. Scanned equipment type will shown, and status will be in progress

7. Fill scanned equipment type

8. Completed equipment type and area will show as done, pop message will also display done.

[Screen details shown:
- Progress tracking from 0/1 to 1/1
- Current time countdown from 1 min to 0 min
- Equipment code C1 with H-Code: UN11111111
- Status changes from "In progress" to "Completed in 14 mins"
- Confirmation message "Nailed it. C1 cage is done."]

v3.5

## Page 7

Loadfill (Adhoc Replenishment) – Previous equipment not scanned and done

Step-by-step process:

1. Scans a new equipment type with previous equipment type in progress
- Shows C2 (H-Code: UN2222222) in progress
- C1 (H-Code: UN1111111) completed in 14 mins

2. Pop up message will ask "Is the previous equipment type done?"
- Options: Yes / No, remove it
- H-Code: UN2222222

3. If previous equipment type was done, select yes and select next
- H-Code: UN2222222

4. Pop up message to start loadfill cage will appear, select "Start loadfill cage"
- Scanned to C3
- H-Code: UN3333333

5. Previous equipment type will show as done. New equipment type will show as in progress and can now be filled and completed
- C3 (H-Code: UN3333333) in progress
- C2 (H-Code: UN2222222) completed in 15 mins
- C1 (H-Code: UN1111111) completed in 14 mins
- "Nailed it. C2 cage is done."

v3.5

## Page 8

Loadfill (Adhoc Replenishment) – Previous equipment not scanned and not done

1. Scans a new equipment type with previous equipment type in progress
- Shows Progress: 1/2
- Current time: 4 min
- C2 (In progress)
- H-Code: UN2222222
- C1 Completed in 14 mins (Done)
- H-Code: UN11111111

2. Pop up message will ask Is the previous equipment type done
- H-Code: UN2222222
- Options: Yes / No, remove it

3. If previous equipment type was not done, select no, remove it and select next
- H-Code: UN22222
- "No, remove it" selected
- "Next" button highlighted

4. Pop up message to start loadfill cage will appear, select Start loadfill cage
- Scanned to C3
- H-Code: UN33333333
- "Start loadfill cage" button shown

5. Previous equipment type will be removed, pop up message will show as removed. New equipment type will show as in progress and can now be filled and completed
- Progress: 1/2
- Current time: 1 min
- C3 (In progress)
- H-Code: UN33333333
- C1 Completed in 14 mins (Done)
- Message: "Removed C2 cage"

v3.5

## Page 9

Loadfill (Adhoc Replenishment) - Screen Messaging

[Three mobile screen interfaces showing different states]

Screen 1:
- Shows "Scanned to C1"
- H-Code: UN11111111
- Message: "Loadfill already completed"
- "Close" button
Caption: Equipment type has already been completed

Screen 2:
- Progress: 1/1
- Current time: 0 min
- C1 Completed in 14 mins (Done)
- H-Code: UN11111111
- Warning message: "Someone is already working on this cage"
Caption: Equipment type is been worked by another team member

Screen 3:
- Progress: 0/0
- "Scan barcode to start"
- "Let's go!"
- Warning message: "C2 cage removed as it is already completed"
Caption: Equipment type has already been completed by another team member

v3.5

## Page 10

Loadfill (Adhoc replenishment) screen information

Screen Elements:
- Total equipment types and progress: Shows as "Progress: 1/2" and "Progress: 2/2"
- Current equipment time: Displays countdown (1 min / 0 min)
- Area: C2
- Equipment and barcode number:
  * C2: H-Cage: UN22222222
  * C1: H-Cage: UN11111111
- Equipment completed time:
  * C1: Completed in 14 mins
  * C2: Completed in 15 mins
- Equipment status: Shows "In progress" or "Done"
- Type in equipment barcode: Input field available
- Timer will display 0 min when no cage is in progress
- All completed equipment types are shown
- Navigation buttons at bottom of screen

Version: v3.5